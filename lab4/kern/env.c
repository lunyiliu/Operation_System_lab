...
env_setup_vm(struct Env *e)
{
	...
	memset(e->env_pgdir, 0, PGSIZE);
		...
		boot_map_region(e->env_pgdir,KERNBASE,(2^32) - KERNBASE,0,PTE_P|PTE_W);
		boot_map_region(e->env_pgdir,UENVS,ROUNDUP(NENV*sizeof(struct Env), PGSIZE),PADDR(envs),PTE_U | PTE_P);
	for (i=PDX(UTEXT);i<PDX(UTOP);i++){
		e->env_pgdir[i] = e->env_pgdir[i] | PTE_P | PTE_W| PTE_U;
	}
	
		for (i=PDX(UTOP);i<1024;i++){
		if(i!=PDX(UVPT))
		e->env_pgdir[i] = e->env_pgdir[i] | PTE_P | PTE_W;
	}
	*/
