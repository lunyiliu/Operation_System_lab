	...
	for (i = 0; i < phnum; i++, ph++) {
		...
		for (va = ROUNDDOWN(ph->p_va, PGSIZE); va != end; offset += PGSIZE, va += PGSIZE) {
			if ((pg = page_lookup(curenv->env_pgdir, (void *)(offset), NULL)) == NULL) 
				panic("SYS_EXEC:Can not find page at %x\n",offset);
			if (page_insert(curenv->env_pgdir, pg, (void *)va, perm) < 0)
				panic("SYS_EXEC:Insertion failed at %x\n",offset);
			page_remove(curenv->env_pgdir, (void *)offset);
		}
	}

	if ((pg = page_lookup(curenv->env_pgdir, (void *)offset, NULL)) == NULL) 
		panic("SYS_EXEC:Can not find Stack , here it is at %x\n",offset);
	if (page_insert(curenv->env_pgdir, pg, (void *)(USTACKTOP - PGSIZE), PTE_P|PTE_U|PTE_W) < 0) 
		panic("SYS_EXEC:Failed to replace the stack at %x\n",offset);
	page_remove(curenv->env_pgdir, (void *)offset);
