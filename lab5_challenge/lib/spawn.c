...
exec(const char *prog, const char **argv)
{
	...
	for (i = 0; i < elf->e_phnum; i++, ph++) {
		...
		if (ph->p_flags & ELF_PROG_FLAG_WRITE)
			perm |= PTE_W;
		cprintf("EXEC:mapped ELF at %x\n",PGOFF(ph->p_va)+stack);
		if ((r = map_segment(0, PGOFF(ph->p_va)+stack, ph->p_memsz,
				     fd, ph->p_filesz, ph->p_offset, perm)) < 0)
			goto error;
		stack += ROUNDUP(ph->p_memsz + PGOFF(ph->p_va), PGSIZE);
	}
	close(fd);
	fd = -1;
	if ((r = init_stack(0, argv, &tf_esp,stack)) < 0)
		return r;
...
copy_shared_pages(envid_t child)
{
	...
	int r;
		...
		{	
			//if(addr==0x20000000){
				//cprintf("addr_PTESHARE:%x,PTE&PTE_SHARE:%d\n",addr,uvpt[PGNUM(addr)] & PTE_SHARE);
			//}
			...
			/*
				else sys_page_map(0, (void*)addr, child, (void*)addr, PTE_U|PTE_P);
			if(addr==USTACKTOP-PGSIZE){
				if ((r = sys_page_alloc(child, (void*)addr, PTE_P|PTE_U|PTE_W)) < 0)
