...
openfile_alloc(struct OpenFile **o)
{
	...
	for (i = 0; i < MAXOPEN; i++) {
		...
		//cprintf("6.pageref(o->o_fd),%d\n",pageref(opentab[i].o_fd));
		switch (pageref(opentab[i].o_fd)) {
		case 0:
				//cprintf("9.pageref(o->o_fd),%d\n",pageref(opentab[i].o_fd));
...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	if ((r = openfile_lookup(envid, req->req_fileid, &o)) < 0)
	return r;
	//cprintf("7.pageref(o->o_fd),%d\n",pageref(o->o_fd));
	/*
	if(!va_is_mapped()){
		return -E_INVAL;
	}
	*/
	//cprintf("&ret->ret_buf[1]:%x\n",&ret->ret_buf[1]);
